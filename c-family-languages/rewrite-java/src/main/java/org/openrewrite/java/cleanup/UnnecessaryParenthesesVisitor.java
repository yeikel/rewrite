...
public class UnnecessaryParenthesesVisitor<P> extends JavaVisitor<P> {
    ...
    public J visitLiteral(J.Literal literal, P p) {
        ...
        CType.Primitive type = l.getType();
        if ((style.getNumInt() && type == CType.Primitive.Int) ||
                (style.getNumDouble() && type == CType.Primitive.Double) ||
                (style.getNumLong() && type == CType.Primitive.Long) ||
                (style.getNumFloat() && type == CType.Primitive.Float) ||
                (style.getStringLiteral() && type == CType.Primitive.String) ||
                (style.getLiteralNull() && type == CType.Primitive.Null) ||
                (style.getLiteralFalse() && type == CType.Primitive.Boolean && l.getValue() == Boolean.valueOf(false)) ||
                (style.getLiteralTrue() && type == CType.Primitive.Boolean && l.getValue() == Boolean.valueOf(true))) {
            if (getCursor().dropParentUntil(J.class::isInstance).getValue() instanceof J.Parentheses) {
                getCursor().putMessageOnFirstEnclosing(J.Parentheses.class, UNNECESSARY_PARENTHESES_MARKER, getCursor());
            }
        }
