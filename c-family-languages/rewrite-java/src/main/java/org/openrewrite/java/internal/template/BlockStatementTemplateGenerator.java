...
public class BlockStatementTemplateGenerator {
        ...
        return Timer.builder("rewrite.template.generate.statement")
                ...
                .record(() -> {
                    ...
                    if (cursor.getValue() instanceof J.MethodDeclaration &&
                        ...
                        before.insert(0, J.printTrimmed(m).trim() + '{');
                        after.append('}');
                    }
    ...
    private void template(Cursor cursor, C prior, StringBuilder before, StringBuilder after, Set<C> templated) {
        ...
        if (j instanceof J.CompilationUnit) {
            ...
            } else if (parent instanceof C.Lambda) {
                before.insert(0, "{\n");
            } else {
                before.insert(0, "{\n");
            }
            ...
            before.insert(0, J.printTrimmed(f).trim());
            ...
            if (t.getResources() != null) {
                ...
                for (J.Try.Resource resource : t.getResources()) {
                    before.insert(0, J.printTrimmed(resource.withPrefix(Space.EMPTY)).trim() + ';');
                }
                before.insert(0, "try(");
            ...
            before.insert(0, J.printTrimmed(l.withBody(null).withPrefix(Space.EMPTY)).trim());
    ...
    private void classDeclaration(@Nullable C prior, StringBuilder before, J.ClassDeclaration parent, Set<C> templated) {
    ...
    private String method(J.MethodDeclaration method) {
        if (method.isAbstract()) {
            return "\n" + J.printTrimmed(method.withPrefix(Space.EMPTY)).trim() + ";\n";
        }

        StringBuilder methodBuilder = new StringBuilder("\n");
        J.MethodDeclaration m = method.withBody(null).withLeadingAnnotations(emptyList()).withPrefix(Space.EMPTY);
        methodBuilder.append(J.printTrimmed(m).trim()).append('{');
        if (method.getReturnTypeExpression() != null && !CType.Primitive.Void.equals(method.getReturnTypeExpression().getType())) {
    ...
    private String variable(J.VariableDeclarations variable, boolean initializer) {
        ...
        for (int i = 0, variablesSize = variables.size(); i < variablesSize; i++) {
            ...
            if (i == 0) {
                ...
                if (nv.getType() instanceof CType.Array) {
                    varBuilder.append("[]");
                }
                varBuilder.append(" ");
            }
