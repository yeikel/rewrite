...
public class CLeftPadded<T> {
    ...
    public CLeftPadded<T> map(UnaryOperator<T> map) {
    ...
    @Nullable
    public static <T> CLeftPadded<T> withElement(@Nullable CLeftPadded<T> before, @Nullable T elements) {
        if (before == null) {
            if (elements == null) {
                return null;
            }
            return new CLeftPadded<>(Space.EMPTY, elements, Markers.EMPTY);
        }
        if (elements == null) {
            return null;
        }
    ...
    public String toString() {
        return "CLeftPadded(before=" + before + ", element=" + element.getClass().getSimpleName() + ')';
    }

    public static <T> CLeftPadded<T> build(T element) {
        return new CLeftPadded<>(Space.EMPTY, element, Markers.EMPTY);
    }
}
